export default function WallpaperCard({ wallpaper, onClick, index = 0 }) {
  const { title, url_preview, resolution_label, width, height, media_type } = wallpaper
  const isMobile   = height > width
  const isAnimated = media_type === 'animated'
  ...
  return (
    ...
    >
      ...
      {isMobile && !isAnimated && (
        <span className="absolute bottom-2 left-2 p-1 rounded bg-black/60 backdrop-blur-sm text-gray-300">
