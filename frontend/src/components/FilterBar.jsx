...
import { Smartphone, Monitor, ChevronDown, Play } from 'lucide-react'
...
export default function FilterBar({ mobileOnly, onMobileToggle, resolution, onResolutionChange, animatedOnly, onAnimatedToggle }) {
    ...
    <div className="bg-surface-850 border-b border-surface-700">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-wrap items-center gap-2 py-2">
          ...
          <span className="text-xs text-gray-600 font-medium hidden md:block">Filters</span>
          <div className="hidden md:block w-px h-4 bg-surface-600" />
          ...
          {/* Clear filters */}
          {(mobileOnly || resolution || animatedOnly) && (
            <button
              onClick={() => { onMobileToggle(); onResolutionChange(null); onAnimatedToggle() }}
              className="text-xs text-gray-500 hover:text-white transition-colors flex items-center gap-1 ml-auto sm:ml-auto">
              Clear filters ✕
            </button>
          )}
