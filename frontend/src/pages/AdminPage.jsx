    ...
    <div className="min-h-screen bg-surface-900 flex items-center justify-center px-4">
      <div className="bg-surface-800 rounded-2xl p-8 w-full max-w-sm border border-surface-700">
        ...
        <input
          ...
          className="w-full px-4 py-2.5 rounded-lg bg-surface-700 border border-surface-600 text-white placeholder-gray-500 text-sm mb-3 outline-none focus:ring-2 focus:ring-brand-400"
        />
...
function ManageTab({ token }) {
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      setResult('Error during deletion.')
    } finally {
      setDeleting(false)
    }
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <input
          ...
          placeholder="Search..."
          className="px-3 py-1.5 rounded-lg bg-surface-700 border border-surface-600 text-white placeholder-gray-500 text-sm outline-none focus:ring-2 focus:ring-brand-400 w-40"
        />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {wallpapers.map(w => (
            ...
            >
              ...
              <div className="absolute top-2 right-2 px-2 py-0.5 rounded text-xs font-mono bg-black/60 text-gray-300">
                {w.resolution_label}
              </div>
              ...
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-end justify-end p-2">
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 transition-all bg-red-500 hover:bg-red-600 text-white text-xs font-semibold px-2 py-1 rounded-lg"
                >
                  Delete
                </button>
              </div>
          ...
          <div className="bg-surface-800 rounded-2xl p-6 max-w-sm w-full border border-surface-700">
            <h3 className="text-white font-bold mb-2">
              {confirm === 'one'
                ? 'Delete wallpaper?'
                : confirm === 'selected'
                  ? `Delete ${selected.size} wallpapers?`
                  : confirm === 'except'
                    ? `Delete all except ${selected.size}?`
...
function ImportTab({ token }) {
  ...
  const selectAllAvailable = () =>
    setSelected(
      new Set(
        results.filter(r => r.meets_quality && !r.already_in_db).map(r => r.id)
      )
    )
  ...
  const doImport = async () => {
    ...
    try {
      ...
      setImportResult({ message: 'Import failed.' })
    } finally {
      setImporting(false)
    }
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      <div className="flex gap-3 mb-6 flex-wrap">
        <input
          ...
          className="flex-1 px-4 py-2.5 rounded-lg bg-surface-700 border border-surface-600 text-white placeholder-gray-500 text-sm outline-none focus:ring-2 focus:ring-brand-400"
        />
      ...
      {importResult && (
        <div className="mb-4 p-4 bg-surface-800 border border-surface-600 rounded-xl text-sm">
          ...
          <button
            ...
            className="mt-2 text-gray-500 hover:text-white text-xs"
          >
            Dismiss
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {visibleResults.map(w => {
            ...
            return (
              <div
                key={w.id}
                onClick={() => isSelectable && toggleSelect(w.id)}
              ...
              >
                ...
                {w.already_in_db && (
                  <div className="absolute bottom-0 left-0 right-0 bg-yellow-500/80 text-black text-xs font-semibold text-center py-1">
                    Already in DB
                  </div>
                )}

                {!w.meets_quality && !w.already_in_db && (
                  <div className="absolute bottom-0 left-0 right-0 bg-red-500/80 text-white text-xs font-semibold text-center py-1">
                    Below quality
                  </div>
                )}
              </div>
