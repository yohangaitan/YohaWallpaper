        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {wallpapers.map(w => (
            <div key={w.id} onClick={() => toggleSelect(w.id)}
              ...
              <img src={w.url_preview} alt={w.title} referrerPolicy="no-referrer" className="w-full h-48 object-cover" loading="lazy" />
...
function ImportTab({ token }) {
  ...
  // Cargar wallpapers aleatorios al entrar
  useEffect(() => { search(1, '') }, [])
          ...
          {visibleResults.map(w => {
            ...
            return (
              <div key={w.id} onClick={() => isSelectable && toggleSelect(w.id)}
                ...
                <img src={w.url_preview} alt={w.id} referrerPolicy="no-referrer" className="w-full h-48 object-cover" loading="lazy" />
                {isSelectable && (
                  <div className={`absolute top-2 left-2 w-5 h-5 rounded border-2 flex items-center justify-center transition-all
